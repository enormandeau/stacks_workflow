...
#### 2.2. Relatedness
  ...
  - Run `vcftools --relatedness --vcf <INPUT_VCF> --out bad_samples` (use `--gzvcf` for comressed VCF files)
    to identify samples with potential errors / problems
  - Plot graph with `./00-scripts/utility_scripts/plot_relatedness_graphs.R bad_samples.relatedness 0.5`
  - Decide on a threshold and create a file with unwanted samples (one sample name per line)
...
```
  - Plot graph with `./00-scripts/utility_scripts/plot_heterozygozity.R bad_samples.het.data`
  - Decide on a threshold and create a file with unwanted samples (one sample name per line)
