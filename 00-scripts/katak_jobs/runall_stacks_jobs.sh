...
p1=$(sbatch "$SCRIPTPATH"/01_cutadapt_job.sh)
p2=$(sbatch "$DEPENDS"$p1 "$SCRIPTPATH"/02_process_radtags_2_enzymes_job.sh)
p3=$(sbatch "$DEPENDS"$p2 ./00-scripts/sequencing_normalization_01.sh)
p4=$(sbatch "$DEPENDS"$p3 ./00-scripts/sequencing_normalization_02.py)
