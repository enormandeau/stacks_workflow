...
d$Color[d$Fis + d$MedRatio * 3 < 0.68] = duplicated
d$Color[d$Fis + d$MedRatio * 8 < 3] = duplicated

# Very low Fis = diverged
d$Color[d$Fis < -0.8] = diverged
d$Color[d$Fis + d$MedRatio * 2 < 0.10] = diverged
d$Color[d$Fis + d$MedRatio * 3 < 0.40] = diverged
d$Color[d$Fis + d$MedRatio * 8 < 1.8] = diverged
...
# Plots
png(paste0(input_file, "_1.png"), width=2400, height=2000)
    plot(d[,1:4], pch=16, cex=1, col=d$Color)
invisible(dev.off())

png(paste0(input_file, "_2.png"), width=2400, height=2000)
    plot(d$PropHet, d$MedRatio, pch=19, cex=1.5, col=d$Color, xlim=c(0, 1), ylim=c(0, 0.8))
invisible(dev.off())

single = d[data$Category == "singleton", ]
png(paste0(input_file, "_3.png"), width=2400, height=2000)
    plot(single$PropHet,
         single$MedRatio,
