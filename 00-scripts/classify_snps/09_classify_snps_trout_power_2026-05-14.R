...
d$MedCovHom[d$MedCovHom > maxMedCovHom * 3] = maxMedCovHom * 3
d$MedCovHet[d$MedCovHet > maxMedCovHet * 3] = maxMedCovHet * 3
...
d$Color[d$Fis + d$MedRatio * 3 < 1.08] = duplicated
...
# Plots
png(paste0(input_file, "_1.png"), width=1600, height=1250)
    plot(d[,1:6], pch=16, cex=0.6, col=d$Color)
invisible(dev.off())

png(paste0(input_file, "_2.png"), width=1600, height=1250)
    plot(d$PropHet, d$MedRatio, pch=19, cex=1.5, col=d$Color, xlim=c(0, 1), ylim=c(0, 0.8))
invisible(dev.off())

single = d[data$Category == "canonical", ]
png(paste0(input_file, "_3.png"), width=1600, height=1250)
    plot(single$PropHet,
         single$MedRatio,
