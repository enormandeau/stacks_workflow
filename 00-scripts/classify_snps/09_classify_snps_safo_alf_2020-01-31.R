...
d$Color[d$MedRatio < 0.33] = lowconf # & d$PropHomRare > 0.00] = lowconf
d$Color[d$MedRatio > 0.67] = lowconf # & d$PropHomRare > 0.00] = lowconf

# Fis is too negative = duplicated
d$Color[d$Fis < -0.05] = duplicated
d$Color[d$Fis + d$MedRatio < 0.40] = duplicated
d$Color[d$Fis + d$MedRatio * 3 < 1.10] = duplicated
d$Color[d$Fis + d$MedRatio * 8 < 2.6] = duplicated
...
d$Color[data$NumHet + data$NumRare < 4] = mas
