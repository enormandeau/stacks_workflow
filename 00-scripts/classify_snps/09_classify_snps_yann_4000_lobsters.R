...
d$Color[d$MedRatio < 0.34] = lowconf # & d$PropHomRare > 0.00] = lowconf
d$Color[d$MedRatio > 0.66] = lowconf # & d$PropHomRare > 0.00] = lowconf

# Fis is too negative = duplicated
d$Color[d$Fis < -0.2] = duplicated
d$Color[d$Fis + d$MedRatio < 0.23] = duplicated
#d$Color[d$Fis + d$MedRatio * 3 < 0.78] = duplicated
#d$Color[d$Fis + d$MedRatio * 8 < 2.3] = duplicated

# Very low Fis = diverged
d$Color[d$Fis < -0.5] = diverged
...
#d$Color[d$MedCovHom < 12 | d$MedCovHet < 12] = highcov
